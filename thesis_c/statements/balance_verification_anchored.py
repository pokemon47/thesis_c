...
class AnchoredBalanceVerificationStatement(ProofStatement):
    name = "balance_verification_anchored"
    ...
    def prepare(self, payloads, baseline_results) -> PreparedStatement:
        ...
        if not baseline.ok or baseline.leaf is None:
            raise ValueError(
                f"Baseline verification failed for anchored balance verification: {baseline.error}"
            )

        balance_prepared = BalanceVerificationStatement().prepare(payloads, baseline_results)
        if str(balance_prepared.public_inputs["hash_name"]).lower() != "keccak256":
            raise ValueError("Anchored balance verification currently supports keccak256 only.")
        ...
        return PreparedStatement(
            ...
            metadata={
                ...
                "proof_mode": "baseline_verified_anchored_balance_verification_keccak_variable_depth",
                "block_hash_binding": "in_circuit",
                "header_authentication": "in_circuit",
                **ANCHORED_BALANCE_VERIFICATION_METADATA,
                "bounds": {
