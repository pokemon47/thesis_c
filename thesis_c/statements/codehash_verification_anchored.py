...
class AnchoredCodeHashVerificationStatement(ProofStatement):
    name = "codehash_verification_anchored"
    ...
    def prepare(self, payloads, baseline_results) -> PreparedStatement:
        ...
        if not baseline.ok or baseline.leaf is None:
            raise ValueError(
                f"Baseline verification failed for anchored codeHash verification: {baseline.error}"
            )

        codehash_prepared = CodeHashVerificationStatement().prepare(payloads, baseline_results)
        if str(codehash_prepared.public_inputs["hash_name"]).lower() != "keccak256":
            raise ValueError("Anchored codeHash verification currently supports keccak256 only.")
        ...
        return PreparedStatement(
            ...
            metadata={
                ...
                "proof_mode": "baseline_verified_anchored_codehash_verification_keccak_variable_depth",
                "block_hash_binding": "in_circuit",
                "header_authentication": "in_circuit",
                **ANCHORED_CODEHASH_VERIFICATION_METADATA,
                "bounds": {
