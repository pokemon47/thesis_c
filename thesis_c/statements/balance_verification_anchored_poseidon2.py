...
ANCHORED_BALANCE_VERIFICATION_METADATA = {
...
class AnchoredPoseidon2BalanceVerificationStatement(ProofStatement):
    name = "balance_verification_anchored_poseidon2"
    ...
    def prepare(self, payloads, baseline_results) -> PreparedStatement:
        ...
        if not baseline.ok or baseline.leaf is None:
            raise ValueError(
                f"Baseline verification failed for anchored Poseidon2 balance verification: {baseline.error}"
            )

        balance_prepared = BalanceVerificationStatement().prepare(payloads, baseline_results)
        if str(balance_prepared.public_inputs["hash_name"]).lower() != "poseidon2":
            raise ValueError("Anchored Poseidon2 balance verification currently supports poseidon2 only.")
        ...
        if header_fixture.state_root.lower() != str(balance_prepared.public_inputs["state_root"]).lower():
            raise ValueError(
                "Anchored header state root does not match the authenticated account state root."
            )

        private_inputs = {
            **balance_prepared.private_inputs,
            "header_anchor": {
        ...
        return PreparedStatement(
            ...
            public_inputs={
                "block_hash": header_fixture.block_hash,
                "state_root": balance_prepared.public_inputs["state_root"],
                "state_root_field": self._state_root_field(
                    str(balance_prepared.public_inputs["state_root"])
                ),
                "account_address": balance_prepared.public_inputs["account_address"],
                "claimed_balance_limbs": balance_prepared.public_inputs["claimed_balance_limbs"],
                "hash_name": balance_prepared.public_inputs["hash_name"],
                "hash_variant_id": balance_prepared.public_inputs["hash_variant_id"],
            },
            ...
            metadata={
                ...
                "proof_mode": "baseline_verified_anchored_balance_verification_poseidon2_variable_depth",
                "block_hash_binding": "in_circuit",
                "header_authentication": "in_circuit",
                **ANCHORED_BALANCE_VERIFICATION_METADATA,
                "bounds": {
