...
ANCHORED_CODEHASH_VERIFICATION_METADATA = {
...
class AnchoredPoseidon2CodeHashVerificationStatement(ProofStatement):
    name = "codehash_verification_anchored_poseidon2"
    ...
    def prepare(self, payloads, baseline_results) -> PreparedStatement:
        ...
        if not baseline.ok or baseline.leaf is None:
            raise ValueError(
                f"Baseline verification failed for anchored Poseidon2 codeHash verification: {baseline.error}"
            )

        codehash_prepared = CodeHashVerificationStatement().prepare(payloads, baseline_results)
        if str(codehash_prepared.public_inputs["hash_name"]).lower() != "poseidon2":
            raise ValueError("Anchored Poseidon2 codeHash verification currently supports poseidon2 only.")
        ...
        if header_fixture.state_root.lower() != str(codehash_prepared.public_inputs["state_root"]).lower():
            raise ValueError(
                "Anchored header state root does not match the authenticated account state root."
            )

        private_inputs = {
            **codehash_prepared.private_inputs,
            "header_anchor": {
        ...
        return PreparedStatement(
            ...
            public_inputs={
                "block_hash": header_fixture.block_hash,
                "state_root": codehash_prepared.public_inputs["state_root"],
                "state_root_field": self._state_root_field(
                    str(codehash_prepared.public_inputs["state_root"])
                ),
                "account_address": codehash_prepared.public_inputs["account_address"],
                "expected_code_hash": codehash_prepared.public_inputs["expected_code_hash"],
                "hash_name": codehash_prepared.public_inputs["hash_name"],
                "hash_variant_id": codehash_prepared.public_inputs["hash_variant_id"],
            },
            ...
            metadata={
                ...
                "proof_mode": "baseline_verified_anchored_codehash_verification_poseidon2_variable_depth",
                "block_hash_binding": "in_circuit",
                "header_authentication": "in_circuit",
                **ANCHORED_CODEHASH_VERIFICATION_METADATA,
                "bounds": {
